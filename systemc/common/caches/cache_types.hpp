...
typedef sc_uint<DMA_WORD_BITS> dma_word_offset_t;
...
template <unsigned REQ_ID_WIDTH> class llc_dma_rsp_out_t {
  ...
  public:
    ...
    inline llc_dma_rsp_out_t &operator=(const llc_dma_rsp_out_t &x)
    {
        coh_msg     = x.coh_msg;
        addr        = x.addr;
        line        = x.line;
    ...
    inline bool operator==(const llc_dma_rsp_out_t &x) const
    {
        return (x.coh_msg == coh_msg && x.addr == addr && x.line == line &&
                x.invack_cnt == invack_cnt && x.req_id == req_id && x.dest_id == dest_id &&
                x.word_offset == word_offset);
    }
    inline friend void sc_trace(sc_trace_file *tf, const llc_dma_rsp_out_t &x,
    ...
    inline friend ostream &operator<<(ostream &os, const llc_dma_rsp_out_t &x)
    {
        os << hex << "(coh_msg: ";
        switch (x.coh_msg) {
            case RSP_DATA: os << "DATA"; break;
...
// dma req in
template <unsigned REQ_ID_WIDTH> class llc_dma_req_in_t {
  ...
  public:
    ...
    inline llc_dma_req_in_t &operator=(const llc_dma_req_in_t &x)
    {
        coh_msg     = x.coh_msg;
        hprot       = x.hprot;
        addr        = x.addr;
    ...
    inline bool operator==(const llc_dma_req_in_t &x) const
    {
        return (x.coh_msg == coh_msg && x.hprot == hprot && x.addr == addr && x.line == line &&
                x.req_id == req_id && x.word_offset == word_offset && x.valid_words == valid_words);
    }
    inline friend void sc_trace(sc_trace_file *tf, const llc_dma_req_in_t &x,
                                const std::string &name)
    {
        sc_trace(tf, x.coh_msg, name + ".coh_msg ");
        sc_trace(tf, x.hprot, name + ".hprot");
    ...
    inline friend ostream &operator<<(ostream &os, const llc_dma_req_in_t &x)
    {
        os << hex << "(coh_msg: ";
        switch (x.coh_msg) {
            case REQ_GETS: os << "GETS"; break;
